...
public class Tablero extends JPanel {
    ...
    private Image fondo; // creando objeto tipo imagen para poner el fondo 
    ...
    private void preInit() {//pre carga las imagenes 
        try { // para solucionar el problema que no se ecuentre la imagen y se pueda cerrar el programa (try = intentar)
            ...
            mgInicial = new ImageIcon("src/graficos/MensajeInicial.jpg").getImage(); // ruta de la imagen de fondo, el método getImage obtine la imagen
    ...
    public void dibujar(Graphics2D g) { //método para dibujar la pelota, las raquetas y los colisionadores de la izquierda y la derecha
        g.setPaint(Color.BLACK);// seleccionamos un color para la pelota (negro por la constante de clase BLACK)
        ...
        g.setPaint(rojo); // seleccionamos el color rojo para la raqueta del jugador 1
        ...
        g.setPaint(verde);// seleccionamos el color verde para la raqueta del jugador 2
    ...
    private void dibujarMarcador(Graphics2D g) { // dibuja el marcador 
        ...
        if (pelota.getMarcador1() == 5) { // condicion para que gane el jugador 1 con un maximo de 5 puntos
            g.drawString("Ganó el Jugador 1", (float) getBounds().getCenterX() - 110, (float) getBounds().getCenterY() - 150);
        ...
        if (pelota.getMarcador2() == 5) { // condicion para que gane el jugador 2 con un maximo de 5 puntos
            g.drawString("Ganó el Jugador 2", (float) getBounds().getCenterX() - 110, (float) getBounds().getCenterY() - 150);
            Pelota.finJuego = true; // termina el juego (finaliza el hilo)
        }
    }
